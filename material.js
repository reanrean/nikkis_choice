...
function chooseSet(){
	...
	if(cate=='自定义'){
		chooseSub += '<input type="text" style="line-height:100%;" id="searchById" placeholder="输入套装名搜索" />';
...
function searchBySetId(){
	...
	if(searchById){
		...
		levelDropNote = table()+tr(tab('套装')+tab('分类'),'style="font-weight:bold;"');
		for (var c in clothes){
		...
		for (var j in searchBySetName){
			if ($.inArray(searchBySetName[j], allSetInCate)<0)
				levelDropNote += tr(tab(ahref(searchBySetName[j],"chgScopeSub2(3,'"+searchBySetName[j]+"')"))+tab('(未分类)'));
		}
