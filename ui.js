...
function row(piece, isShoppingCart) {
	...
	$lineTop.append(td(render(csv[14]) + (render(csv[15])? '/' : '') + render(csv[15]), 'source'));
	if (isShoppingCart) {
		if (piece.id) {
...
function clothesNameDeriv_search(setName) {
	...
	$clothesNameA.click(function () {
		...
		for (var i in clothes){
			if(clothes[i].set == setName || clothes[i].setRef == setName + '·套' || clothes[i].setRef == setName + '·染' || clothes[i].setRef == setName + '·基') {
                setContent.push(i);
            }
			//search setBonus
